// Code generated by go-swagger; DO NOT EDIT.

package operations

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the generate command

import (
	"net/http"

	middleware "github.com/go-openapi/runtime/middleware"
	strfmt "github.com/go-openapi/strfmt"
	swag "github.com/go-openapi/swag"
)

// DefaultHandlerFunc turns a function with the right signature into a default handler
type DefaultHandlerFunc func(DefaultParams) middleware.Responder

// Handle executing the request and returning a response
func (fn DefaultHandlerFunc) Handle(params DefaultParams) middleware.Responder {
	return fn(params)
}

// DefaultHandler interface for that can handle valid default params
type DefaultHandler interface {
	Handle(DefaultParams) middleware.Responder
}

// NewDefault creates a new http.Handler for the default operation
func NewDefault(ctx *middleware.Context, handler DefaultHandler) *Default {
	return &Default{Context: ctx, Handler: handler}
}

/*Default swagger:route GET / default

Default default API

*/
type Default struct {
	Context *middleware.Context
	Handler DefaultHandler
}

func (o *Default) ServeHTTP(rw http.ResponseWriter, r *http.Request) {
	route, rCtx, _ := o.Context.RouteInfo(r)
	if rCtx != nil {
		r = rCtx
	}
	var Params = NewDefaultParams()

	if err := o.Context.BindValidRequest(r, route, &Params); err != nil { // bind params
		o.Context.Respond(rw, r, route.Produces, route, err)
		return
	}

	res := o.Handler.Handle(Params) // actually handle the request

	o.Context.Respond(rw, r, route.Produces, route, res)

}

// DefaultOKBody default o k body
// swagger:model DefaultOKBody
type DefaultOKBody struct {

	// data
	Data string `json:"data,omitempty"`

	// success
	Success bool `json:"success,omitempty"`
}

// Validate validates this default o k body
func (o *DefaultOKBody) Validate(formats strfmt.Registry) error {
	return nil
}

// MarshalBinary interface implementation
func (o *DefaultOKBody) MarshalBinary() ([]byte, error) {
	if o == nil {
		return nil, nil
	}
	return swag.WriteJSON(o)
}

// UnmarshalBinary interface implementation
func (o *DefaultOKBody) UnmarshalBinary(b []byte) error {
	var res DefaultOKBody
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*o = res
	return nil
}
